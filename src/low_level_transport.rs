...
use std::{io, mem};
use thrust::protocol::{Serializer, Deserializer, ThriftSerializer, ThriftDeserializer};
...
/// reactor.
pub struct LowLevelThriftTransport<T, D, S> {
    inner: T,
    deserializer: D,
    serializer: S
    read_buffer: Vec<u8>,
...
pub fn new_thrift_transport<T, D, S>(inner: T, deserializer: D, serializer: S) -> LowLevelFlockTransport<T> 
    where T: Io,
D: Deserializer + ThriftDeserializer,
S: Serializer + ThriftSerializer,
{
    LowLevelThriftTransport {
        inner: inner,
    ...
    where T: Io
{
    ...
    fn read(&mut self) -> Poll<InFrame, io::Error> {
        loop {
